...
void Circulo_Atribui_Centro (tCirculo c, tPonto p){
    ...
    // // Aloca um novo ponto dinamicamente
    // tPonto novoCentro = Pto_Cria(Pto_Acessa_x(p), Pto_Acessa_y(p));

    // // Atribui o novo ponto ao centro do círculo
    // c->centro = novoCentro;

    c->centro = p;

    //Pto_Apaga(novoCentro);
